  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={toggle}
          className="flex h-9 w-9 items-center justify-center rounded-xl text-slate-600 transition-colors hover:bg-slate-50 hover:text-slate-900 md:hidden"
        >
          <Menu className="h-5 w-5" />
        </button>
        {session && (
          <span className="hidden sm:block text-sm text-slate-600">
            Ola, <span className="text-slate-900 font-medium">{firstName}</span>!
          </span>
        )}
      </div>
      ...
      <div className="hidden sm:flex flex-1 max-w-xl mx-auto">
        <div className="relative w-full">
          ...
          <input
            ...
            className={cn(
              "w-full rounded-xl bg-slate-50 border border-slate-200 py-2.5 pl-10 pr-4",
              "text-sm text-slate-900 placeholder:text-slate-400",
              "transition-all duration-200",
              "focus:bg-white focus:border-violet-300 focus:outline-none focus:ring-1 focus:ring-violet-500/30"
            )}
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <motion.button
          ...
          className="flex h-9 w-9 items-center justify-center rounded-full bg-violet-100 text-xs font-semibold text-violet-700 transition-transform hover:scale-105"
        >
          {userInitials}
        </motion.button>
      </div>
